...
class Request {
    constructor(userId, password) {
        ...
        this.header = {
            credentials: 'include',
            headers: {
                'Authorization': 'Basic ' + new Buffer.from(userId + ':' + password).toString('base64')
            }
        }
    }
    ...
    search(user, platform = 'pc', mode = 'fast') {
        return new Promise((resolve, reject) => {
            if (typeof platform !== 'string' || typeof user !== 'string' || typeof mode !== 'string') {
                reject('Wrong data type');
            }
            fetch(`https://api.statsdb.net/r6/${platform}/search/${user}/${mode}`, this.header)
                .then(res => res.json())
                .then(data => {
                    resolve(data)
                })
    ...
    player(user, platform = 'pc') {
        return new Promise((resolve, reject) => {
            ...
            fetch(`https://api.statsdb.net/r6/${platform}/player/${user}`, this.header)
            .then(res => res.json())
            .then(data => {
                resolve(data)
            })
